...
<div
	...
	<?php if ( ! ( $disable_wishlist && ! is_user_logged_in() ) ): ?>
		<div
			class="hint--bounce yith-wcwl-add-button <?php echo is_product() ? 'hint--top' : 'hint--left'; ?> <?php echo '' . ( $exists && ! $available_multi_wishlist ) ? 'hide' : 'show' ?><?php echo( lezada_get_option( 'animated_wishlist_on' ) ? ' animated-wishlist' : '' ); ?>"
			aria-label="<?php esc_attr_e( 'Add to wishlist', 'lezada' ) ?>"
		...
		<div
			class="hint--bounce yith-wcwl-wishlistexistsbrowse <?php echo is_product() ? 'hint--top' : 'hint--left'; ?> <?php echo '' . ( $exists && ! $available_multi_wishlist ) ? 'show' : 'hide' ?>"
			aria-label="<?php echo esc_attr( $browse_wishlist_text ) ?>"
