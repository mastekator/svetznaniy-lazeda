	...
	<div id="comments">
		<h2 class="woocommerce-Reviews-title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				...
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'lezada' ) ),
					esc_html( $count ),
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php _e( 'There are no reviews yet.', 'lezada' ); ?></p>
			...
			<div id="review_form">
				...
				$comment_form = array(
					'title_reply'         => have_comments() ? __( 'Add a review',
						'lezada' ) : sprintf( __( 'Be the first to review &ldquo;%s&rdquo;', 'lezada' ),
						get_the_title() ),
					'title_reply_to'      => __( 'Leave a Reply to %s', 'lezada' ),
				...
				if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
					$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating',
							'lezada' ) . '</label><select name="rating" id="rating" aria-required="true" required>
							<option value="">' . esc_html__( 'Rate&hellip;', 'lezada' ) . '</option>
							<option value="5">' . esc_html__( 'Perfect', 'lezada' ) . '</option>
							<option value="4">' . esc_html__( 'Good', 'lezada' ) . '</option>
							<option value="3">' . esc_html__( 'Average', 'lezada' ) . '</option>
							<option value="2">' . esc_html__( 'Not that bad', 'lezada' ) . '</option>
							<option value="1">' . esc_html__( 'Very poor', 'lezada' ) . '</option>
						</select></div>';
				}
