...
$body_font     = lezada_get_option( 'primary_font' );
$primary_color = lezada_get_option( 'primary_color' );
...
if ( ! lezada_get_option( 'catalog_mode_on' ) && is_null( $catalog_enable ) ) {
...
<head>
	...
	<?php add_filter( 'wp_title', function () {
		return __( 'Product Comparison', 'lezada' );
	} ); ?>
	...
	<link rel="stylesheet" href="<?php echo LEZADA_THEME_URI; ?>/assets/libs/font-awesome/css/font-awesome.min.css"
	      type="text/css" media="all">
	<link rel="stylesheet"
	      href="<?php echo LEZADA_THEME_URI; ?>/assets/libs/pixeden-stroke-7-icon/css/pe-icon-7-stroke.min.css"
	      type="text/css" media="all">
	...
	<style type="text/css">
		...
		table.compare-list .add-to-cart td a {
			...
			color: <?php echo esc_html(lezada_get_option('link_hover_color'))?>;
			margin: 0;
			padding: 0;
			-webkit-transition: all .3s ease-in-out;
			-moz-transition: all .3s ease-in-out;
		...
		table.compare-list .add-to-cart td a.loading:before {
			content: url(<?php echo LEZADA_THEME_URI . '/assets/images/loading.svg';?>);
			position: absolute;
			top: -10px;
			left: 50%;
			line-height: 0;
...
<h1 class="title">
	...
	<?php if ( ! $is_iframe ) : ?><a class="close"
	                                 href="#"><?php _e( 'Close window [X]', 'lezada' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) )
	...
	<?php if ( empty( $products ) ) : ?>
		...
		<tr class="no-products">
			<td><?php esc_html_e( 'No products added in the compare table.', 'lezada' ) ?></td>
		</tr>
			...
			<?php foreach ( $products as $product_id => $product ) :
				...
				<td class="<?php echo esc_attr( $product_class ); ?>">
					<a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>"
					   data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php _e( 'Remove', 'lezada' ) ?>
				</td>
			<?php endforeach ?>
		</tr>
				...
				<?php foreach ( $products as $product_id => $product ) :
					...
					<td class="<?php echo esc_attr( $product_class ); ?>"><?php
						switch ( $field ) {
							...
							case 'image':
								echo '<div class="image-wrap">' . wp_get_attachment_image( $product->fields[ $field ], apply_filters( 'lezada_compare_image_size', 'yith-woocompare-image' ) ) . '</div>';
								break;
