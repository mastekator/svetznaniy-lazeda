	...
	class ReduxFramework_repeater {
		...
		public function render() {
			...
			if ( isset ( $this->value['Redux_repeater_data'] ) && is_array( $this->value['Redux_repeater_data'] ) && ! empty ( $this->value['Redux_repeater_data'] ) ) {
				...
				foreach ( $repeaters as $repeater ) {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . __( 'Delete', 'lezada' ) . ' ' . $this->field['item_name'] . '</a>';
					}
					echo '</div>';
					echo '</fieldset>';
					echo '</table>';
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . __( 'Add', 'lezada' ) . ' ' . $this->field['item_name'] . '</a><br/>';
			}
