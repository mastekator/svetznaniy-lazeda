...
if (!class_exists('Lezada_Helper')) {
    ...
    class Lezada_Helper
    {
        ...
        {
            add_action('wp_ajax_lezada_ajax_search', array($this, 'ajax_search'));
            add_action('wp_ajax_nopriv_lezada_ajax_search', array($this, 'ajax_search'));
            add_filter('posts_where', array($this, 'title_like_posts_where'), 10, 2);

            add_action('wp_ajax_lezada_ajax_load_more', array($this, 'ajax_load_more'));
            add_action('wp_ajax_nopriv_lezada_ajax_load_more', array($this, 'ajax_load_more'));
        }
        ...
        public static function get_all_menus($allow_empty = false, $vc = false)
        {
            ...
            if ($allow_empty) {
                $menus = array(esc_html__('-- Select a menu --', 'lezada') => '');
            }
        ...
        public static function get_registered_sidebars($default_option = false, $disable_option = false)
        {
            ...
            if ($default_option) {
                $sidebars['default'] = esc_html__('Default', 'lezada');
            }

            if ($disable_option) {
                $sidebars['no'] = esc_html__('Disable', 'lezada');
            }
        ...
        {
            return array(
                'fullwidth' => LEZADA_FRAMEWORK_URI . '/assets/images/1c.png',
                'content-sidebar' => LEZADA_FRAMEWORK_URI . '/assets/images/2cr.png',
                'sidebar-content' => LEZADA_FRAMEWORK_URI . '/assets/images/2cl.png',
            );
        }
        ...
        public static function get_config($setting = '')
        {
            ...
            $path = LEZADA_THEME_DIR . DS . 'assets' . DS . 'json' . DS . 'config.json';
        ...
        public static function social_icons($custom = true, $colors = false)
        {
            ...
            $networks = array(
                ...
                'envelope-o' => array(
                    'label' => esc_html__('Email Address', 'lezada'),
                    'color' => '#000000',
                ),
            ...
            if ($custom) {
                $networks['custom'] = array('label' => esc_attr__('Custom', 'lezada'), 'color' => '');
            }
        ...
        public static function get_the_title()
        {
            ...
            if (is_front_page() && !is_singular('page')) {
                $title = esc_html__('Our Blog', 'lezada');
            ...
            if (is_search()) {
                ...
                $title = apply_filters('lezada_page_title',
            ...
            if (is_tag() || is_category() || is_date() || is_author()) {
                ...
                if (is_author()) {
                    ...
                    // Daily archive title.
                    $title = sprintf(esc_html__('Daily Archives: %s', 'lezada'), get_the_date());
                } elseif (is_month()) {
                    // Monthly archive title.
                    $title = sprintf(esc_html__('Monthly Archives: %s', 'lezada'),
                        get_the_date(esc_html_x('F Y', 'Page title monthly archives date format', 'lezada')));
                } elseif (is_year()) {
                    // Yearly archive title.
                    $title = sprintf(esc_html__('Yearly Archives: %s', 'lezada'),
                        get_the_date(esc_html_x('Y', 'Page title yearly archives date format', 'lezada')));
                } else {
            ...
            if (is_404()) {
                ...
                $title = esc_html__('404: Page Not Found', 'lezada');
            ...
            if ($post_id) {
                ...
                if (is_singular('post')) {
                    $title = get_post_meta($post_id,
                        'lezada_post_title_on_top',
                        true) ? get_the_title($post_id) : esc_html__('Our Blog', 'lezada');
                } elseif (is_singular('product')) {
                    $title = get_post_meta($post_id,
                        'lezada_product_title_on_top',
                ...
                // Custom meta title.
                if ($custom_title = get_post_meta($post_id, 'lezada_custom_page_title', true)) {
                    $title = $custom_title;
                }
            }
            ...
            if (class_exists('WooCommerce') && (is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy())) {
                ...
                if (apply_filters('woocommerce_show_page_title', true) && !is_product()) {
                    ...
                    if (is_shop() && $custom_title = get_post_meta($post_id, 'lezada_custom_page_title', true)) {
                        $title = $custom_title;
                    }
            ...
            return apply_filters('lezada_page_title', $title);;
        }
        ...
        public function ajax_search()
        {
            ...
            if (!empty($_REQUEST['query'])) {
                ...
                $post_type = isset($_REQUEST['post_type']) ? $_REQUEST['post_type'] : lezada_get_option('search_post_type');
                $posts_per_page = isset($_REQUEST['limit']) ? $_REQUEST['limit'] : lezada_get_option('search_limit');
                ...
                $thumb_size = apply_filters('lezada_search_thumb_size', 'lezada-search-thumb');
                        ...
                        if (!empty($search_ids)) {
                            ...
                            $suggestions = array(
                                'suggestions' => array(
                                    array(
                                        'id' => -1,
                                        'value' => pll__('Sorry, but nothing matched your search terms. Please try again with different keywords'),
                                        'url' => '',
                                    ),
                                ),
                            );
                ...
                if ($posts->have_posts()) {
                    ...
                    if (intval($posts->found_posts) > $posts_per_page) {
                        ...
                        $suggestions[] = array(
                            'id' => -2,
                            'value' => esc_html__('View All', 'lezada'),
                            'url' => esc_url(home_url('/')) . $query_str,
                            'target' => apply_filters('lezada_search_view_all_target', '_blank'),
                        );
                    }
        ...
        public function ajax_load_more()
        {
            ...
            if ($post_type == 'post') {
                ...
                $shortcode_post_class = isset($_POST['columns']) ? 'col-lg-' . Lezada_VC::calculate_column_width($_POST['columns']) : 'col-lg-3';
            ...
            if ($post_type == 'product') {
                ...
                $query = Lezada_Woo::get_products_by_datasource($data_source, $atts, $args);

                add_filter('lezada_shop_products_columns',
                    function () {
                ...
                add_filter('lezada_shop_products_classes',
                    function () {
                        return 'adding-item';
                    });
            }
            ...
            if ($query->have_posts()) {
                ...
                while ($query->have_posts()) {
                    ...
                    if ($post_type == 'post') {
                        ...
                        add_filter('lezada_blog_image_size',
                            function () {
                                return ($view == 'grid') ? 'lezada-post-grid' : 'lezada-single-thumb';
                            });
        ...
        public static function get_active_sidebar($shop = false)
        {
            ...
            $page_id = Lezada_Helper::get_the_ID();

            // DEFAULT SIDEBAR CONFIG - Page.
            $default_sidebar_config = lezada_get_option('page_sidebar_config');
            $sidebar = get_post_meta($page_id, 'lezada_page_custom_sidebar', true);

            // DEFAULT SIDEBAR CONFIG - Blog.
            if (is_archive() || is_author() || is_category() || is_home()) {
                $default_sidebar_config = lezada_get_option('archive_sidebar_config');
                $sidebar = lezada_get_option('archive_sidebar');
            }

            // For search page
            if (is_search()) {
                $default_sidebar_config = lezada_get_option('search_sidebar_config');
                $sidebar = lezada_get_option('search_sidebar');
            }

            // DEFAULT SIDEBAR CONFIG - Single Post.
            if (is_singular('post')) {
                $sidebar_config = get_post_meta($page_id, 'lezada_post_sidebar_config', true);
                $sidebar = get_post_meta($page_id, 'lezada_post_custom_sidebar', true);
            }
            ...
            if ($shop) {
                ...
                if (class_exists('WooCommerce') && (is_shop() || is_product_taxonomy()) && !is_singular('product') && !is_search()) {
                    $sidebar_config = lezada_get_option('shop_sidebar_config');
                    $sidebar = lezada_get_option('shop_sidebar');
                }

                // In single product page
                if (is_product()) {
                    $sidebar_config = get_post_meta($page_id, 'lezada_product_sidebar_config', true);
                    $sidebar = get_post_meta($page_id, 'lezada_product_custom_sidebar', true);

                    if ($sidebar_config == 'default') {
                        $sidebar_config = lezada_get_option('product_sidebar_config');
                        $sidebar = lezada_get_option('product_sidebar');
                    }
                }
        ...
        public static function get_rev_sliders()
        {
            global $wpdb;
            $revsliders = array(
                '' => esc_html__('Choose a slider', 'lezada'),
            );
