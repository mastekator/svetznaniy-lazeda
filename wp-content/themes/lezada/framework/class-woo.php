...
if ( ! class_exists( 'Lezada_Woo' ) ) {
	...
	class Lezada_Woo {
		...
		public function __construct() {
			 ...
			 *****************************************************************************************/
			// Wishlist AJAX
			add_action( 'wp_ajax_lezada_get_wishlist_fragments',
				array(
					$this,
					'get_wishlist_fragments',
				) );
			add_action( 'wp_ajax_nopriv_lezada_get_wishlist_fragments',
				array(
					$this,
					'get_wishlist_fragments',
				) );
			add_action( 'wp_ajax_lezada_remove_wishlist_item',
				array(
					$this,
					'remove_wishlist_item',
				) );
			add_action( 'wp_ajax_nopriv_lezada_remove_wishlist_item',
				array(
					$this,
					'remove_wishlist_item',
				) );
			add_action( 'wp_ajax_lezada_undo_remove_wishlist_item',
				array(
					$this,
					'undo_remove_wishlist_item',
				) );
			add_action( 'wp_ajax_nopriv_lezada_undo_remove_wishlist_item',
				array(
					$this,
					'undo_remove_wishlist_item',
				) );
			...
			add_filter( 'woocommerce_add_to_cart_fragments',
				...
				10 );
			add_action( 'wp_ajax_lezada_remove_cart_item',
				array(
					$this,
					'remove_cart_item',
				) );
			add_action( 'wp_ajax_nopriv_lezada_remove_cart_item',
				array(
					$this,
					'remove_cart_item',
				) );
			add_action( 'wp_ajax_lezada_undo_remove_cart_item',
				array(
					$this,
					'undo_remove_cart_item',
				) );
			add_action( 'wp_ajax_nopriv_lezada_undo_remove_cart_item',
				array(
					$this,
					'undo_remove_cart_item',
				) );
			...
			// Quick view AJAX
			add_action( 'wp_ajax_lezada_quick_view',
				array(
					$this,
					'quick_view',
				) );
			add_action( 'wp_ajax_nopriv_lezada_quick_view',
				array(
					$this,
					'quick_view',
				) );
			add_action( 'lezada_after_page_container',
				array(
					$this,
					'add_quick_view_container',
				) );
			add_action( 'wp_ajax_lezada_ajax_add_to_cart',
				array(
					$this,
					'ajax_add_to_cart',
				) );
			add_action( 'wp_ajax_nopriv_lezada_ajax_add_to_cart',
				array(
					$this,
					'ajax_add_to_cart',
				) );
			...
			//Stock
			add_shortcode( 'woocommerce_after_shop_loop_item', 'product_stock' );
			...
			// Shop toolbar
			add_action( 'lezada_after_page_title', array( $this, 'shop_toolbar' ), 30 );
			add_action( 'lezada_after_page_title', array( $this, 'filter_widgets' ), 31 );
			...
			add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'product_buttons' ), 20 );
			...
			add_action( 'woocommerce_single_product_summary',
				function () {
					echo Lezada_Templates::single_navigation();
				},
				6 );
			...
			add_filter( 'woocommerce_output_related_products_args',
				function ( $args ) {
					...
					$args['posts_per_page'] = lezada_get_option( 'product_related' );
					$args['columns']        = 4;
			...
			add_action( 'woocommerce_after_add_to_cart_quantity',
				array( $this, 'open_block_action_single_product' ),
				10 );

			add_action( 'woocommerce_after_add_to_cart_button',
				array( $this, 'close_block_action_single_product' ),
				15 );
		...
		function open_block_action_single_product() {
			global $product;
		...
		function close_block_action_single_product() {
			global $product;
			if ( ! $product->is_type( 'external' ) ) {
				echo '</div>';
			}
		...
		private function wishlist_widget() {
			...
			?>
			<p class="widget_wishlist_title"><?php esc_html_e( 'Wishlist', 'lezada' ); ?>
					...
					foreach ( $products as $p ) :
						...
						<li class="wishlist_item"
							...
							<a href="<?php echo esc_url( $remove_url ); ?>" class="remove"
							   title="<?php esc_html_e( 'Remove this product', 'lezada' ) ?>">&times;</a>
							...
							<div class="wishlist_item_right">
								...
								<?php if ( ! $product->is_in_stock() ) : ?>
									<p class="outofstock"><?php esc_html_e( 'Out of stock', 'lezada' ); ?></p>
								<?php endif; ?>
								<?php if ( $product->is_in_stock() && lezada_get_option( 'wishlist_add_to_cart_on' ) ) {
			...
			if ( ! empty( $products ) ) {
				...
				if ( lezada_get_option( 'wishlist_target' ) ) {
					$target = '_blank';
				}
				?>
				<a href="<?php echo esc_url( $wl_link ); ?>"
				   target="<?php echo esc_attr( $target ); ?>"
				   class="button btn-view-wishlist"><?php esc_html_e( 'View Wishlist', 'lezada' ); ?></a>
				<?php
			}
		}
		...
		public static function get_quickview_image_size() {
			...
			return array(
				apply_filters( 'lezada_quickview_image_width', $image_width ),
				apply_filters( 'lezada_quickview_image_height', $image_height ),
			);
		}
		...
		public function shop_toolbar() {
			...
			$column_switcher = lezada_get_option( 'column_switcher' );

			if ( ! lezada_get_option( 'shop_toolbar' ) || ! self::is_shop() ) {
				return;
			}

			ob_start();

			$columns         = isset( $_COOKIE['lezada_shop_columns'] ) && ! is_null( $_COOKIE['lezada_shop_columns'] ) ? intval( $_COOKIE['lezada_shop_columns'] ) : intval( get_option( 'woocommerce_catalog_columns',
				4 ) );
			$tabs            = lezada_get_option( 'product_tabs' );
			$sort            = lezada_get_option( 'product_sorting' );
			$full_width_shop = lezada_get_option( 'full_width_shop' );

			if ( $tabs ) {
				$type      = lezada_get_option( 'product_tabs_type' );
				$tabs_html = array(
				...
				if ( $type == 'tags' ) {
					$tags = trim( lezada_get_option( 'product_tabs_tags' ) );
					...
					$tab_categories = lezada_get_option( 'product_tabs_categories' );
					$args           = array(
					...
					$groups = lezada_get_option( 'product_tabs_groups' );
			...
			<div class="shop-toolbar <?php echo '' . ($column_switcher === '0' ) ? ' disable-switcher' : ''; ?>">
				<div class="container<?php echo '' . ( $full_width_shop != 'basic' ) ? ' wide' : ''; ?>">
					<div class="row">
						<div
						...
						<div class="col-md-3 col-lg-4 controls">
							<ul class="toolbar-control">
								...
								<li class="col-switcher">
									<a href="#" class="<?php echo '' . $columns == 3 ? 'active' : '' ?>" data-col="5">
										<i class="ti-layout-grid4-alt"></i>
									</a>
									<a href="#" class="<?php echo '' . $columns == 4 ? 'active' : '' ?>" data-col="4">
										<i class="ti-layout-grid3-alt"></i>
									</a>
									<a href="#" class="<?php echo '' . $columns == 5 ? 'active' : '' ?>" data-col="3">
										<i class="ti-layout-grid2-alt"></i>
									</a>
								</li>
								<?php if ( is_active_sidebar( 'shop-filter' ) && lezada_get_option( 'product_filter' ) ) : ?>
									<li class="filter-button">
										<a href="#" class="open-filters"><?php _e( 'Filters', 'lezada' ); ?></a>
									</li><!-- .l-filter-buttons -->
								<?php endif; ?>
							</ul>
						</div>
		...
		public function filter_widgets() {
			...
			if ( is_active_sidebar( 'shop-filter' ) && lezada_get_option( 'product_filter' ) ) :
				ob_start();
				?>
		...
		public function categories_row() {
			...
			if ( 'subcategories' === $display_type || 'both' === $display_type ) {
				...
				$layout        = lezada_get_option( 'categories_layout' );
				$data_carousel = ' data-carousel="' . lezada_get_option( 'categories_columns' ) . '"';
		...
		public static function product_categories_menu() {
			...
			$args = apply_filters( 'lezada_product_categories_menu_args', $args );
			...
			?>
			<div class="shop-menu">
				<a href="#" class="show-categories-menu"><?php esc_html_e( 'Categories', 'lezada' ) ?></a>
				<ul class="product-categories-menu">
					<?php if ( is_product_category() || is_product_tag() ) : ?>
		...
		public function product_buttons() {
			...
			<div class="product-buttons">
				<?php
				echo Lezada_Woo::wishlist_button();
				echo Lezada_Woo::compare_button();
				echo Lezada_Woo::quick_view_button();
				?>
			</div>
			<?php
		}
		...
		public function template_loop_product_thumbnail() {
			...
			$size = apply_filters( 'lezada_product_loop_thumbnail_size', $size );
		...
		public static function wishlist_button() {
			if ( class_exists( 'YITH_WCWL' ) && 'yes' == get_option( 'yith_wcwl_enabled' ) && lezada_get_option( 'shop_wishlist_on' ) ) {
		...
		public static function quick_view_button() {
			...
			if ( lezada_get_option( 'shop_quick_view_on' ) && ! wp_is_mobile() ) : ?>
				<div class="quick-view-btn hint--left hint-bounce"
				     aria-label="<?php esc_html_e( 'Quick View', 'lezada' ); ?>"
				     data-pid="<?php echo esc_attr( $id ) ?>"
				     data-pnonce="<?php echo esc_attr( wp_create_nonce( 'lezada_quick_view' ) ); ?>">
					<a href="#" aria-label="<?php esc_html_e( 'Quick View', 'lezada' ); ?>" rel="nofollow">
						<?php esc_html_e( 'Quick View', 'lezada' ); ?>
					</a>
				</div>
			<?php endif;
		...
		public static function compare_button() {
			...
			$button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Compare', 'lezada' ) );
			...
			if ( class_exists( 'YITH_Woocompare' ) && lezada_get_option( 'shop_compare_on' ) && ! wp_is_mobile() ) : ?>
				<div class="compare-btn hint--bounce hint--left"
				     aria-label="<?php esc_html_e( 'Compare', 'lezada' ); ?>">
					<?php
					//die();
		...
		public function add_quick_view_container() {
			echo '<div id="woo-quick-view" class="' . ( lezada_get_option( 'animated_quick_view_on' ) ? 'animated-quick-view' : '' ) . '"></div>';
		}
		...
		public function ajax_add_to_cart() {
			...
			if ( $atc ) {
				...
				if ( $sold_indv == 'yes' ) {
					$response            = array( 'result' => 'fail' );
					$response['message'] = esc_html__( 'Sorry, that item can only be added once.', 'lezada' );
				} else {
		...
		public static function product_instagram() {
			...
			$hashtag = get_post_meta( Lezada_Helper::get_the_ID(), 'lezada_product_hashtag', true );
			...
			$number = apply_filters( 'lezada_product_instagram_number', 8 );
			$col    = apply_filters( 'lezada_product_instagram_columns', 4 );
			$class  = Lezada_Helper::get_grid_item_class( array(
				'xl' => $col,
				'lg' => 4,
				'md' => 2,
				'sm' => 1,
			...
			<div class="product-instagram">
				<div class="container">
					<p><?php printf( wp_kses( __( 'Tag your photos with <a href="%s" target="%s">%s</a> on Instagram.',
							...
							$hashtag,
							apply_filters( 'lezada_product_instagram_link_target', '__blank' ) ); ?></p>
					<?php

					$media_array = Lezada_Instagram::get_instance()->scrape_instagram( $hashtag, $number );
						...
						<div class="tm-instagram-pics row">
							<?php foreach ( $media_array as $item ) : ?>
								<div class="item <?php echo esc_attr( $class ) ?>">
									<div class="item-info">
										<span><?php esc_html_e( 'View on Instagram', 'lezada' ); ?></span>
									</div>
									...
									<div class="overlay">
										<a href="<?php echo esc_url( $item['link'] ); ?>"
										   target="<?php echo apply_filters( 'lezada_product_instagram_link_target',
											   '_blank' ); ?>"><?php esc_html_e( 'View on Instagram', 'lezada' ) ?></a>
									</div>
		...
		public static function get_products_by_datasource( $data_source, $atts, $args = array() ) {
			...
			$transient_name = 'lezada_wc_loop' . substr( md5( json_encode( $args ) . $data_source ),
