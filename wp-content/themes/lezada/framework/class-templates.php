 ...
 */
if ( ! class_exists( 'Lezada_Templates' ) ) {

	class Lezada_Templates {

		/**
		 * Insight_Templates constructor.
		 */
		public function __construct() {
			add_action( 'lezada_base_loop', array( $this, 'base_loop' ) );
		}
		...
		public function base_loop() {
			...
			<?php if ( have_posts() ) : ?>
				<div class="<?php echo esc_attr( $container_class ); ?> row" data-atts="<?php echo esc_attr( json_encode( $atts ) ); ?>">
					<?php $index = 0; ?>
		...
		public static function post_meta( $atts = array() ) {
			...
			$default = apply_filters( 'lezada_post_meta',
				array(
					'author'   => 0,
					'cats'     => 0,
					'date'     => 1,
			...
			<div class="entry-meta">
				<?php if ( get_post_type() === 'post' || get_post_type() === 'page' ) : ?>
					...
					<?php if ( is_sticky() && $atts['sticky'] ) : ?>
						<span class="meta-featured-post"><?php esc_html_e( 'Featured', 'lezada' ) ?></span>
					<?php endif; ?>

					<?php // Author. ?>

					<?php if ( lezada_get_option( 'post_meta_author' ) ) {

						if ( $atts['author'] && $author_id ) { ?>
							<span class="meta-author byline">
								<span class="vcard author">
							<?php esc_html_e( 'By', 'lezada' ) ?>
		...
		public static function language_switcher() {
			...
			$language_switcher_on = lezada_get_option( 'topbar_language_switcher_on' );
		...
		private static function polylang_switcher() {
				...
				if ( ! empty( $langs ) ) :
					...
					if ( function_exists( 'pll_the_languages' ) ) :
						...
						if ( ! empty ( $langs ) ) {
							...
							$html = apply_filters( 'lezada_polylang_switcher', $html );
						}
						?>
			...
			do_action( 'lezada_polylang_switcher' );
		...
		private static function wpml_switcher() {
			...
			if ( ! empty( $langs ) ) {
				...
				foreach ( $langs as $l ) {
					...
					$html = apply_filters( 'lezada_wpml_switcher', $html );
				}
			}
			?>
			<div
			...
			do_action( 'lezada_wpml_switcher' );
		...
		public static function currency_switcher() {
			...
			$currency_switcher_on = lezada_get_option( 'topbar_currency_switcher_on' );
		...
		private static function woocs_switchers() {
			...
			$dash             = apply_filters( 'lezada_woocs_switchers_dash', ' - ' );
			...
			$html = apply_filters( 'lezada_woocs_switchers', $html );
			?>
		...
		private static function woo_wpml_switcher() {
			...
			$switcher_style = apply_filters( 'lezada_currency_switcher_style', 'dropdown' );
			$format         = apply_filters( 'lezada_currency_template', '%symbol% - %code%' );
			...
			$html = apply_filters( 'lezada_woo_wpml_switcher', $html );
		...
		public static function social_links( $classes = array() ) {
			...
			$icons           = lezada_get_option( 'icon' );
			$icon_classes    = lezada_get_option( 'icon_class' );
			$urls            = lezada_get_option( 'url' );
			$titles          = lezada_get_option( 'title' );
			$custom_classes  = lezada_get_option( 'custom_class' );
			$tooltip         = lezada_get_option( 'tooltip' );
			$open_in_new_tab = lezada_get_option( 'social_open_in_new_tab' );
			$labels          = Lezada_Helper::social_icons( false );
		...
		private function pagination() {
			...
			if ( $links ) :
				...
				$pagination = lezada_get_option( 'archive_pagination' );
				$display_type           = lezada_get_option( 'archive_display_type' );
				...
				if ( $pagination == 'default' ) :
					...
					?>
					<div class="lezada-loadmore-wrap lezada-pagination posts-pagination"
					     data-atts="<?php echo esc_attr( json_encode( $load_more_atts ) ); ?>">
						<a href="#"
						   class="button lezada-loadmore-btn load-on-<?php echo '' . ( $pagination == 'more-btn' ) ? 'click' : 'scroll'; ?>">
							<?php esc_html_e( 'more', 'lezada' ); ?></a>
					</div>
					<?php
				endif;
			endif;
		...
		public static function time_ago( $type = 'post' ) {
			$date = ( $type == 'comment' ) ? get_comment_time( 'U' ) : get_post_time( 'U' );
			$diff = (int) abs( time() - $date );
			$diff = round( $diff / DAY_IN_SECONDS );

			if ( $diff >= 15 && $diff <= 30 ) { // show full date and time if the date time is 15 days and 30 days
				return ( $type == 'comment' ) ? get_comment_date() : get_the_date();
			} else {
				return human_time_diff( $date ) . " " . __( 'ago', 'lezada' );
			}
		}
			...
			<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
				...
				<div class="thecomment">
					...
					<div class="comment-text">
						...
						<span class="date"><?php echo Lezada_Templates::time_ago( 'comment' ); ?></span>
						...
						<div class="comment-actions">
							<?php comment_reply_link( array_merge( $args,
								array(
									...
									'reply_text' => '<span class="ion-reply"></span>' . esc_html__( 'Reply', 'lezada' ),
								) ) ); ?>
							<?php edit_comment_link( '<span class="ion-android-create"></span>' . esc_html__( 'Edit',
									'lezada' ) ); ?>
						</div>
					</div>
		...
		public static function mobile_menu() {
			...
			if ( Lezada_Coming_Soon::is_coming_soon_page() ) {
				return '';
			}
			...
			<div class="site-mobile-menu">
				...
				if ( class_exists( 'Lezada_Walker_Nav_Menu' ) && has_nav_menu( 'primary' ) ) {
					$args['walker'] = new Lezada_Walker_Nav_Menu();
				}
				...
				if ( lezada_get_option( 'header_login' ) ) {
					echo self::header_block_header_login();
				}

				if ( lezada_get_option( 'mobile_menu_social' ) ) {
		...
		public static function back_to_top() {
			...
			if ( ! lezada_get_option( 'back_to_top_on' ) ) {
				return;
			}
		...
		public static function cookie_notice() {
			...
			if ( ! lezada_get_option( 'cookie_on' ) ) {
				return;
			}

			ob_start();

			if ( ! isset( $_COOKIE['lezada_cookie_notice_accepted'] ) || $_COOKIE['lezada_cookie_notice_accepted'] != 'yes' ) :
				?>
				<div class="cookie-wrapper"
				     data-expires="<?php echo esc_attr( lezada_get_option( 'cookie_expires' ) ); ?>">
					<div class="cookie-inner">
						<div class="cookie-message">
							<?php echo do_shortcode( lezada_get_option( 'cookie_message' ) ); ?>
						</div>
		...
		public static function header_offcanvas_btn() {
			...
			<div class="offcanvas-btn hidden-lg-down on-<?php echo esc_attr( $offcanvas_position ); ?>">
				<a href="#"><i class="ion-navicon"></i><span><?php esc_html_e( 'Open', 'lezada' ) ?></span></a>
			</div>
			<?php
		...
		public static function header_offcanvas() {
			...
			if ( ! lezada_get_option( 'offcanvas_button_on' ) || lezada_get_option( 'header' ) == 'vertical' ) {
				return '';
			}
			...
			$offcanvas_position = lezada_get_option( 'offcanvas_position' );
			$offcanvas_action   = lezada_get_option( 'offcanvas_action' );
			$offcanvas_sidebar  = lezada_get_option( 'offcanvas_sidebar' );

			if ( is_page() ) {
				$offcanvas_sidebar = get_post_meta( Lezada_Helper::get_the_ID(),
					'lezada_offcanvas_custom_sidebar',
					true );
			}
		...
		public static function header_block_logo() {
			...
			$header_overlap = lezada_get_option( 'header_overlap' );
			$header_scheme  = lezada_get_option( 'header_scheme' );

			$custom_logo = get_post_meta( $id, 'lezada_custom_logo', true );

			$logo     = $o_logo = $o_logo_mobile = $logo_mobile = LEZADA_IMAGES . '/logo.png';
			$logo_retina = LEZADA_IMAGES . '/logo-retina.png';
			$logo_alt = $logo_mobile_alt = LEZADA_IMAGES . '/logo-alt.png';

			$logo_uploaded            = lezada_get_option( 'logo' );
			$logo_retina_uploaded     = lezada_get_option( 'logo_retina' );
			$logo_alt_uploaded        = lezada_get_option( 'logo_alt' );
			$logo_mobile_uploaded     = lezada_get_option( 'logo_mobile' );
			$logo_mobile_alt_uploaded = lezada_get_option( 'logo_mobile_alt' );

			if ( isset( $logo_uploaded['url'] ) && $logo_uploaded['url'] ) {
				$logo = $o_logo = is_ssl() ? str_replace('http:', 'https:', $logo_uploaded['url']) : $logo_uploaded['url'];
			}
			...
			if ( is_singular( 'page' ) && $custom_logo == 'on' ) { // On single page
				$logo            = $o_logo = get_post_meta( $id, 'lezada_logo', true );
				$logo_alt        = get_post_meta( $id, 'lezada_logo_alt', true );
				$logo_mobile     = get_post_meta( $id, 'lezada_logo_mobile', true );
				$logo_mobile_alt = get_post_meta( $id, 'lezada_logo_mobile_alt', true );
			}
		...
		public static function header_block_site_menu() {
			...
			$header = lezada_get_option( 'header' );
			$align  = lezada_get_option( 'site_menu_align' );
			$style  = lezada_get_option( 'site_menu_style' );
			...
			$menu_disabled = get_post_meta( Lezada_Helper::get_the_ID(), 'lezada_disable_site_menu', true ) == 'on' || is_404();
			...
			<div class="site-menu hidden-lg-down <?php echo implode( ' ', $menu_classes ) ?>">
				...
				if ( ! $menu_disabled && ! Lezada_Coming_Soon::is_coming_soon_page() ) {
					...
					if ( class_exists( 'Lezada_Walker_Nav_Menu' ) && has_nav_menu( 'primary' ) && $header != 'vertical' ) {
						$args['walker'] = new Lezada_Walker_Nav_Menu();
		...
		public static function header_block_search() {
			...
			<div class="header-search">
				<a href="#" class="toggle">
					...
					<span><?php esc_attr_e( 'Search', 'lezada' ) ?></span>
				</a>
			</div>
		...
		public static function search_form() {
			...
			$post_type     = lezada_get_option( 'search_post_type' );
			$categories_on = lezada_get_option( 'search_categories_on' );
			$ajax_search   = lezada_get_option( 'search_ajax_on' );
			$min_chars     = lezada_get_option( 'search_min_chars' );
			...
			<div class="search-form-wrapper">
				<form action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" role="search"
					...
					<?php if ( $categories_on ) {
						...
						$search_child_cats = apply_filters( 'lezada_search_child_cats', true );
					...
					<button type="submit" id="search-btn"
					        title="<?php esc_attr_e( 'Search', 'lezada' ); ?>"><?php esc_html_e( 'Search',
							'lezada' ); ?></button>
				</form>
		...
		public static function header_block_wishlist() {
			...
			if ( ! class_exists( 'YITH_WCWL' ) || ! lezada_get_option( 'wishlist_on' ) ) {
				return;
			}

			ob_start();

			$wishlist_icon = lezada_get_option( 'wishlist_icon' );

			$wl_count = YITH_WCWL()->count_products();

			?>
			<div class="header-wishlist">
				<a href="#" class="toggle" aria-label="<?php esc_attr_e( 'Wishlist', 'lezada' ) ?>">
		...
		public static function header_block_cart() {
			...
			if ( ! class_exists( 'WooCommerce' ) || ! lezada_get_option( 'minicart_on' ) ) {
				return;
			}

			ob_start();

			$cart_icon = lezada_get_option( 'minicart_icon' );

			?>
			<div class="header-minicart">
				<a href="#" class="toggle" aria-label="<?php esc_attr_e( 'Shopping Cart', 'lezada' ) ?>">
					<i class="<?php echo esc_attr( $cart_icon ) ?>" aria-hidden="true"></i>
					<span class="minicart-text">
						<?php echo Lezada_Woo::get_cart_count(); ?>
						<span class="minicart-title"><?php esc_html_e( 'My Cart', 'lezada' ) ?></span>
						<?php echo Lezada_Woo::get_cart_total(); ?>
					</span>
				</a>
		...
		public static function header_block_header_login() {
			...
			$text = is_user_logged_in() ? esc_html__( 'My Account', 'lezada' ) : esc_html__( 'Log in',
				'lezada' );

			if ( lezada_get_option( 'header_login_position' ) == 'between' ) {
				$text = is_user_logged_in() ? esc_html__( 'My Account', 'lezada' ) : esc_html__( 'Log in / Register',
					'lezada' );
			}

			$class = '';
			if ( lezada_get_option( 'header_login_icon' ) == '1' ) {
				$class = 'login-icon';
			}
			...
			<div class="header-login <?php echo esc_attr( $class ) ?>">
				<a href="<?php echo get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ); ?>"><i
						class="ion-android-person"></i><span><?php echo esc_html( $text ); ?></span></a>
			</div>
			<?php
			return ob_get_clean();
		}
		...
		public static function page_title() {
			...
			if ( is_404() || Lezada_Coming_Soon::is_coming_soon_page() ) {
				$page_title_on = false;
			}

			// Apply filters and return
			$page_title_on = apply_filters( 'lezada_page_title_on', $page_title_on );
			...
			if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() ) && lezada_get_option( 'shop_toolbar' ) ) {
				$classes[] = 'page-title-shop';
			}

			if ( lezada_get_option( 'disable_parallax' ) ) {
				$classes[] = 'no-parallax';
			}
			...
			if ( $page_title_on || ( ! $page_title_on && lezada_get_option( 'shop_toolbar' ) == '1' ) ) {
				do_action( 'lezada_before_page_title' );
				?>
				<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
					<div class="container">
						<div class="row">
							<div class="col-xs-12">
								<h1 class="entry-title"><?php echo Lezada_Helper::get_the_title(); ?></h1>

								<?php
								if ( is_page() && lezada_get_option( 'page_meta' ) ) {
				...
				<?php
				do_action( 'lezada_after_page_title' );
			}
		...
		public static function page_subtitle() {
			...
			// Posts & Pages.
			if ( $meta = get_post_meta( Lezada_Helper::get_the_ID(), 'lezada_subtitle', true ) ) {
				$subheading = $meta;
			}
			...
			if ( is_search() ) {
				$subheading = esc_html__( 'You searched for:',
						'lezada' ) . ' &quot;' . esc_html( get_search_query( false ) ) . '&quot;';
			}

			// Author.
			if ( is_author() ) {
				$subheading = esc_html__( 'This author has written',
						'lezada' ) . ' ' . get_the_author_posts() . ' ' . esc_html__( 'articles', 'lezada' );
			}
			...
			if ( function_exists( 'is_shop' ) && is_shop() && get_option( 'woocommerce_shop_page_id' ) == Lezada_Helper::get_the_ID() ) {
				$subheading = get_post_meta( Lezada_Helper::get_the_ID(), 'lezada_subtitle', true );
			}
			...
			// Apply filters and return.
			return apply_filters( 'lezada_post_subheading', $subheading );

		}

		public static function breadcrumbs( $args = '' ) {

			$breadcrumbs_on = lezada_get_option( 'breadcrumbs' );
		...
		public static function single_navigation() {
			...
			$image_size   = 'lezada-single-navigation';
			$next_product = false;
			$prev_product = false;
