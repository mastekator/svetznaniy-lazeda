...
if ( ! class_exists( 'Lezada_Extras' ) ) {
	...
	class Lezada_Extras {
		...
		public function body_classes( $classes ) {
			...
			$box_mode_on = lezada_get_option( 'box_mode_on' );

			$header           = lezada_get_option( 'header' );
			$header_overlap   = lezada_get_option( 'header_overlap' );
			$page_extra_class = get_post_meta( Lezada_Helper::get_the_ID(), 'lezada_page_extra_class', true );
		...
		public function extra_info() {
			if ( isset( $_GET['debug'] ) && 'true' === $_GET['debug'] ) {
				...
				if ( LEZADA_THEME_NAME != $child_theme->name ) {
				...
				$wpdebug   = defined( 'WP_DEBUG' ) ? WP_DEBUG ? esc_html__( 'Enabled', 'lezada' ) : esc_html__( 'Disabled', 'lezada' ) : esc_html__( 'Not Set', 'lezada' );
				$jquchk    = wp_script_is( 'jquery', 'registered' ) ? $GLOBALS['wp_scripts']->registered['jquery']->ver : esc_html__( 'n/a', 'lezada' );
				$fr_page   = $frontpage ? get_the_title( $frontpage ) . ' (ID# ' . $frontpage . ')' . '' : esc_html__( 'n/a', 'lezada' );
				$fr_post   = $frontpage ? get_the_title( $frontpost ) . ' (ID# ' . $frontpost . ')' . '' : esc_html__( 'n/a', 'lezada' );
				?>
				<!--
=====WORDPRESS DATA=====<?php echo "\n"; ?>
* WordPress: v<?php echo esc_html( $wp_version ) . "\n"; ?>
* Theme: <?php echo LEZADA_THEME_NAME; ?> v<?php echo LEZADA_THEME_VERSION; ?><?php echo "\n"; ?>
...
-->
		...
		public function add_quote_text_field() {
			...
			<div class="vp-pfui-elm-block">
				<label for="vp-pfui-format-quote-text"><?php esc_html_e( 'Quote', 'lezada' ); ?></label>
